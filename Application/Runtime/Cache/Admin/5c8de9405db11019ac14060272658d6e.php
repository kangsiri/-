...
<div class="data-list clear">请选择商品分类：
	...
	<table border="1">
		...
		<?php if(is_array($goods["list"])): foreach($goods["list"] as $key=>$v): ?><tr><td><?php echo ($v["identifier"]); ?></td>
			...
			<td class="center"><a href="/dyk/Admin/Recycle/undel/gid/<?php echo ($v["gid"]); ?>/cid/<?php echo ($cid); ?>">还原</a> <a href="/dyk/Admin/Goods/revise/gid/<?php echo ($v["gid"]); ?>/cid/<?php echo ($cid); ?>" target="_blank" >修改</a> <a href="/dyk/Admin/Recycle/del/gid/<?php echo ($v["gid"]); ?>/cid/<?php echo ($cid); ?>">删除</a></td>
...
<script>
	$("select").change(function(){
		window.location.href = "/dyk/Admin/Recycle/index/cid/" + $(this).val();
	});
	$("tr:odd").addClass("odd");
</script></div>
	</div>
