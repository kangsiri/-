...
<div class="title-btn left"><a href="/dyk/Admin/Category/add">添加分类</a></div>
...
href="/dyk/Admin/Category/revise/cid/<?php echo ($v["cid"]); ?>">修改</a> <a href="#"
...
<script>
	$(".data-tree").on("click","#new_div :button",function(){
		...
		var pid = $("#new_pid").val();
		var div = $(this);
		$.post("/dyk/Admin/Category/addAjax",{cname: cname, pid: pid},function(msg){
			if (msg === false) {alert('添加失败');return false;}
			var html = "<div>" + cname;
			html += ' <a href="#" onclick="add_line(this,'+msg+')">添加</a> ';
			html += '<a href="/dyk/Admin/Category/revise/cid/'+msg+'")">修改</a> ';
	...
	function del(obj, id) {
		$.get("/dyk/Admin/Category/remove", {cid: id}, function (msg) {
			if (msg.flag === true) {
				$(obj).parent().remove();
			}else{
				alert(msg.msg);
